...
```{r}
  ggplot(arcos_county_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="cornflower blue") 
...
md_pills_per_year <- arcos_county_pills_per_year %>%
...
```{r}
  ggplot(md_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="cornflower blue") +
  labs(x="Year", y="Total pills", title="Maryland Pills per Year", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
...
```{r}
# Baltimore City had a 2006 spike, then fall in 2007 and steadily climbed through 2012. The state overall had a steady climb and peaked in 2011.
...
``` {r}
state_pills_population <- state_pills_2012 %>%
...
state_pills_population <- state_pills_population %>%
