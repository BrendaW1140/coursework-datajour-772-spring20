...
medicare_opioids_working <- medicare_opioids %>%
  ...
  rename(prescribing_rate = medicare_opioid_prescribing_rate) %>%
  rename(urban_prescribing_rate = medicare_urban_opioid_prescribing_rate) %>%
  rename(rural_prescribing_rate = medicare_rural_opioid_prescribing_rate) %>%
  mutate(prescribing_rate = na_if(prescribing_rate, "NO VALUE PROVIDED")) %>%
  mutate(urban_prescribing_rate = na_if(urban_prescribing_rate, "NO VALUE PROVIDED")) %>%
  mutate(rural_prescribing_rate = na_if(rural_prescribing_rate, "NO VALUE PROVIDED")) %>%
  separate(county, into=c("county","state"), sep=", ") %>%
  mutate(county = tolower(county)) %>%
