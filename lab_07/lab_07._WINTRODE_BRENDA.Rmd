...
```{r}
# Comment: When we calculate pills per population, we will not have a population count for the counties in "not in pop" table. If a county is not in "not in pills" that is not a problem because they didn't get any pills.
```
...
print(head(county_geodata_shifted))
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Pills Per Person By State", subtitle = "2009 population, U.S. Census", caption = "Source: U.S. Census ACS") +
...
geo_pills_population_county_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Pills Per Person By County", subtitle = "2009 population, U.S. Census", caption = "Source: U.S. Census ACS") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)

#The data reveals Appalachia as a hot spot for high counts of pills per person by county in 2009, but so does Nevada, Northern Arizona, Northern Califormia and the upper Midwest.
...
# Cecil, Kent and Allegheny counties had high pills per person count from 2008-2011. The lower Eastern Shore's count peaked in 2011.
