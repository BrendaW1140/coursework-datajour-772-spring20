...
mingo %>%
 ...
 mutate(cardinal_difference = if_else(reporter_name == "CARDINAL HEALTH", "CARDINAL HEALTH, INC.", "CARDINAL HEALTH #110, LLC." )) %>%
 select(cardinal_difference, everything())

#mingo %>%
 #  mutate(cardinal_difference = if_else(reporter_name == "CARDINAL HEALTH", "CARDINAL HEALTH, INC.", "CARDINAL #HEALTH    110, LLC.") %>%
#  select(cardinal_difference, everything())
  
#Hi Sean, I hope there's a history in here that you can see all of the various attempts. I have been at this one task for almost an hour. I'm going to have to move on. Output appeared at one point, but it was not 100% what I wanted.
...
#I would want to take the three current names and condense them into one name.
