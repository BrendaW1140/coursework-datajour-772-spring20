...
#I suspect the buyer totals are aggregated over a number of years and the buyer annual shows totals by each individual year.

#What about your best guess for why buyer_addresses is higher than buyer_totals? 

#It's possible there are more than one Rite Aid Pharmacy in a town. There will be more individual pharmacy addresses if that is true.
...
``` {r}
 ...
 pharmacy_location_totals <- buyer_addresses %>%
  left_join(buyer_annual_by_year, by="buyer_dea_no") %>%

#VA Consolidated - This is a by-mail pharmacy run by the Veterans Administration. I would be interested in looking into the government oversight of said establishments.
...
practitioner_totals <- buyer_addresses %>%
...
practitioner_totals %>%
  group_by(buyer_name) %>%
  summarise(total_pills) %>%
  arrange(desc(totals_pills))
```
