...
#This crashed my R before. I had to restart the project. Skipping this time.
```
...
* Each column is stored in one of two formats: "chr", or strings of characters, generally used for text; and "dbl", a number format on which mathematical operations can be performed.  Do you see any examples of a column stored as a "dbl" that doesn't really need to be, because we would never do math on it? 

REPORTER_ZIP is listed as a dbl, but we would never do math with it.

Are there any columns stored as character that you think should be stored in another format entirely? No

* Based on the information here, how are missing values represented in this data set? "null"
...
#The DEA closed a Baltimore pharmacy and indicted four of its employees on federal drug charges, laundering and tax evasion. Four employees of Newcare Home Health Services in Baltimore ordered 6,133,600 from drug distributors and sold them over the internet.
...
``` {r}
baltimore %>%
  group_by(reporter_name, buyer_city) %>%
